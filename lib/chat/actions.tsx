...
import { ollama } from '@/lib/ollama/ollama-provider'
...
async function regenUserMessage(messageId: string) {
  'use server'
...
if (messageIndex !== -1) {
    messages.splice(messageIndex+1, messages.length - messageIndex);
} else {
...
export const getUIStateFromAIState = (aiState: Chat) => {
  return aiState.messages
    ...
    .map((message, index) => ({
      ...
      display:
       message.role === 'user' ? (
          <UserMessage messageId={message.id}>{message.content as string}</UserMessage>
